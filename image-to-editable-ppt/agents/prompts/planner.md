...
6. Trace the complete connector topology before positioning nodes. Preserve curved cycles, branches, merges, direction, and arrowhead destinations. A semantic connector declares both `from_id` and `to_id`, has a destination arrowhead, and ends close to both node boundaries without entering their text-safe areas. A purely decorative stem, divider, leader accent, or underline must omit both IDs; do not invent node semantics merely because a line visually touches a card. Use `geometry: curve` with explicit cubic control points when connector endpoints must meet specific node boundaries; use `geometry: arc` only for a true quarter-arc. Do not replace a closed loop with disconnected or visibly floating segments.
7. Do not replace a distinctive source icon with a Unicode glyph, emoji, letter, generic polygon, or visually simplified symbol. Treat gradients, highlights, inner shadows, soft shadows, transparency, 3D treatment, texture, photographic detail, and irregular source-specific contours as complexity signals, not as an automatic command to crop. Rebuild regular dots, lines, rings, halos, basic shadows, and repeatable geometric patterns with native PowerPoint objects when they preserve the source. Use a sanitized text-free SVG for vector decoration or vector texture that needs complex paths, masks, or gradients. Use a crop for photographs, portraits, source-specific raster/noise texture, rendered objects, and artwork whose identity depends on photographic detail, inner shadow, or 3D treatment.
8. Crop a polished or rendered source icon as PNG when those effects determine its identity. Use sanitized SVG only for text-free vector complexity that can preserve the source appearance. A larger visual structure may combine separate native, SVG, and cropped subcomponents; inventory each subcomponent as its own representation decision instead of flattening the whole structure into one asset. Keep surrounding text and card structure native.
...
12. A texture is not automatically a screenshot. Use native objects for regular geometric patterns, sanitized SVG for text-free vector texture, and a local background crop only for source-specific raster texture that contains no required text and whose every edge can blend into the native slide. Never introduce a visible rectangular seam.
13. Make every `representation_decisions` target resolve to the emitted layout and asset specifications. A `crop` decision requires a matching image element, crop entry, cropped PNG/JPEG manifest entry, and identical boundary policy. An `svg` decision requires a matching image element and SVG manifest entry. `contains_readable_text` describes text retained inside an emitted asset, not native text elsewhere in the region: every native decision must set it to `false`, and only an inseparable brand mark asset may set it to `true` with the matching manifest exemption.
...
16. Keep crop boxes inside the source image. Write every crop box as absolute source edges `[left, top, right, bottom]`, where right and bottom are exclusive edges—not width and height. `output` is a filename only such as `icon_target.png`; never include `assets/`, a drive letter, or a path separator.
...
- Valid native card: `visual_kind: simple_vector`, `complexity_signals: ["soft_shadow"]`, `selected_representation: native`, `asset_ids: []`, `contains_readable_text: false`.
- Valid native texture: `visual_kind: texture`, `complexity_signals: ["texture", "transparency"]`, `selected_representation: native` when it is a regular dot/line pattern reproduced by native objects.
- Valid SVG texture: `visual_kind: texture`, `complexity_signals: ["gradient", "transparency"]`, `selected_representation: svg` when it is text-free vector decoration.
- Required crop: `visual_kind: texture`, `complexity_signals: ["texture", "photographic_detail"]`, `selected_representation: crop` for source-specific raster or noise texture.
- Invalid native icon: `visual_kind: complex_icon`, `complexity_signals: ["soft_shadow", "three_dimensional"]`, `selected_representation: native`.
- Valid crop: `box_px: [120, 80, 248, 208]`, `output: "icon_target.png"`; `[120, 80, 128, 128]` is not an x/y/width/height tuple.
- Invalid output: `assets/icon_target.png`; the manifest path may contain `assets/`, but the crop `output` may not.
- Valid semantic connector: both `from_id` and `to_id` plus a destination arrowhead. Valid decorative line: neither ID. A one-sided ID pair is always invalid.
